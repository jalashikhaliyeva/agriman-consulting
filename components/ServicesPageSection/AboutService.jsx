...
function AboutService({ serviceData }) {
  ...
  // If no service data is provided, show a placeholder
  if (!serviceData) {
    return (
      <div className="bg-white rounded-4xl p-6 flex items-center justify-center h-full">
        <div className="text-center text-neutral-400">
          <p className="text-xl mb-3">Bu xidmətə aid məlumat tapılmadı</p>
          <svg
            width="56"
            height="56"
            viewBox="0 0 28 28"
  ...
  return <Template serviceData={serviceData} />;
}
    ...
    <div className="bg-white rounded-4xl p-6">
      <div className="flex flex-col gap-5 font-arimo">
        <div className="flex flex-row items-center gap-5 font-arimo">
          ...
          <h2 className="text-lg md:text-4xl font-bold">{serviceData.title}</h2>
        </div>
      ...
      {/* Continuous flow grid layout */}
      {attributes.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-8">
          {/* Row 1: Text 1, Image 1, Text 2 */}
          <div className="flex flex-col h-48 w-full md:w-[300px]">
            {attributes[0] && (
        ...
        // Fallback with a placeholder message when no attributes
        <div className="flex items-center justify-center h-48 w-full mt-8">
          <div className="text-center text-neutral-400">
            <p className="text-xl">Bu xidmətə aid məlumat tapılmadı</p>
          </div>
        </div>
      )}

      {/* Assuming ContactBanner is imported at the top */}
      <div className="mt-8">{/* ContactBanner component would be here */}</div>
    </div>
  );
}

// Template 2: Full-width image with sections below
    ...
    <div className="bg-white rounded-4xl p-6">
      <div className="flex flex-col gap-5 font-arimo">
        <div className="flex flex-row items-center gap-5 font-arimo">
          ...
          <h2 className="text-4xl font-bold">{serviceData.title}</h2>
        </div>
          ...
          {serviceData.attributes.map((attr, index) => (
            <div key={index} className="flex flex-row gap-5 mt-8 w-full">
              ...
              <div className="flex w-[65%] flex-col gap-4">
                ...
                <p className="text-neutral-400 text-base">{attr.description}</p>
              </div>
            </div>
          ))}
        </div>
...
// Template 3: Alternating image and text blocks
    ...
    <div className="bg-white rounded-4xl p-6">
      <div className="flex flex-col gap-5 font-arimo">
        <div className="flex flex-row items-center gap-5 font-arimo">
          ...
          <h2 className="text-lg md:text-4xl font-bold text-wrap">
            {serviceData.title}
          </h2>
        </div>
            ...
            >
              {index % 2 === 0 ? (
                <>
                  ...
                  <div className="flex w-full md:w-[35%] flex-col gap-4">
                    <p className="text-black text-xl md:text-2xl font-medium font-archivo">
                      {attr.title}
                    </p>
                    <p className="text-neutral-400 text-base">
                      {attr.description}
                ...
                <>
                  <div className="flex items-start md:items-end w-full md:w-[35%] flex-col gap-4 order-1 md:order-none">
                    <p className="text-black text-xl md:text-2xl font-medium font-archivo">
                      {attr.title}
                    </p>
                    <p className="text-neutral-400 md:text-end text-base">
                      {attr.description}
                    </p>
                  </div>
      ...
      <div className="flex flex-col gap-3 pt-5">
        ...
        <p className="text-neutral-400">{serviceData.description_2}</p>
      </div>
